...
# Criar arquivo HTML com WebSocket client avançado e integração Eleven Labs
...
<body>
  <div class="container">
    ...
    <div class="controls">
      ...
      <button id="startBtn" disabled>🎤 Falar</button>
      <button id="stopBtn" class="secondary" style="display:none;">⏹️ Parar</button>
    </div>
    ...
    <div class="info-panel">
      <div class="info-title">ℹ️ Informações da Conexão</div>
      <div class="info-item">🆔 ID do Cliente: <span id="clientId">-</span></div>
      <div class="info-item">🔗 Status WebSocket: <span id="wsStatus">Desconectado</span></div>
      <div class="info-item">⏱️ Última Atividade: <span id="lastActivity">-</span></div>
    </div>
  </div>
  ...
  <script>
    ...
    if (SpeechRecognition) {
      ...
      addMessage('sistema', 'Seu navegador não suporta reconhecimento de voz', 'system');
    }
    ...
    function connectToWebSocket() {
      ...
      ws.onmessage = (event) => {
        ...
        switch(data.type) {
          ...
          case 'userActivity':
            console.log(`📢 Atividade: ${data.activity}`);
            break;
      ...
      ws.onerror = (error) => {
        ...
        addMessage('erro', 'Erro na conexão WebSocket', 'system');
      };
    ...
    if (recognition) {
      recognition.onresult = (event) => {
        ...
        addMessage('você', speechResult, 'user');
    ...
    async function generateSpeech(text) {
      try {
        ...
        addMessage('erro', 'Erro ao gerar áudio', 'system');
        avatar.classList.remove('speaking');
        hideVisualizer();
        updateStatus('connected');
      }
...
with open('index_realtime.html', 'w', encoding='utf-8') as f:
    f.write(html_file)

print("✅ Arquivo index_realtime.html criado!")
print("\n" + "="*60)
